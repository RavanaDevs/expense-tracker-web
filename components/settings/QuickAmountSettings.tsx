...
export default function QuickAmountSettings({
  isOpen,
  onClose,
}: QuickAmountSettingsProps) {
  const quickAmountSettings = useSettingsStore(
    (state) => state.settings.quickAmounts
  );
  const updateQuickAmounts = useSettingsStore(
    (state) => state.updateQuickAmounts
  );
  const [quickAmounts, setQuickAmounts] =
